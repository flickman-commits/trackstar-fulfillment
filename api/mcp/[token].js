 ...
 *
 * The connector dialog takes a URL and nothing else: no header field, no
 * credential field. Remote MCP servers are expected to either run a full OAuth
 * flow or carry their secret in the address. OAuth is the right long-term
 * answer and is not a thing to build at midnight, so for now the token is a
 * path segment.
 *
 * That is a real tradeoff and worth naming rather than hiding: URLs get logged,
 * copied, and pasted into places headers do not go. It is mitigated by keeping
 * this server READ-ONLY - nothing here can clear research, change a price,
 * message a customer or touch an order - so a leaked URL exposes operational
 * data, not the ability to break anything. Adding the agent's repair tools
 * means revisiting this first.
 *
 * ─── Protocol ───
 *
...
import { TOOLS, callTool } from '../../server/services/mcpTools.js'
